...
export function CriterionCard({ criterion, onClick }: CriterionCardProps) {
	return (
		...
		>
			<Stack gap="xs">
				<Group gap="xs">
					<Tooltip label={getCriterionAccessLabel(criterion.workspace_id, criterion.task_id)}>
					...
					<Tooltip label={stageLabels[criterion.stage ?? 'null']}>
						{criterion.stage === 'PROJECT_DOC' && <IconFileDescription size={16} color="gray" />}
						{criterion.stage === 'CODEBASE' && <IconCode size={16} color="gray" />}
						{criterion.stage === 'MANUAL' && <IconSchool size={16} color="gray" />}
						{criterion.stage === null && <IconStack2 size={16} color="gray" />}
					</Tooltip>
				</Group>
				...
				<Text lineClamp={2}>{criterion.description}</Text>
			</Stack>
		</Card>
	);
}
