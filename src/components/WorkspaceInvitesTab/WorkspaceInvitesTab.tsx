...
function InviteModal({ opened, onClose, workspaceId, editRule }: InviteModalProps) {
  ...
  const [role, setRole] = useState<'TEACHER' | 'STUDENT'>((editRule?.role === 'TEACHER' || editRule?.role === 'STUDENT') ? editRule.role : 'STUDENT');
  const [password, setPassword] = useState('');
  const [expiredAt, setExpiredAt] = useState<Date | null>(editRule?.expired_at ? new Date(editRule.expired_at) : null);
            ...
            {rules.map((rule) => (
              <Table.Tr key={rule.id}>
                ...
                <Table.Td>
                  {rule.expired_at ? (
                    <Text size="sm">
                      {new Date(rule.expired_at).toLocaleString('ru-RU')}
                    </Text>
                  ) : (
