...
export function CriteriaEditPage() {
  ...
  useEffect(() => {
    if (criterion) {
      ...
      setIsPublic(criterion.is_public);
    }
  }, [criterion]);

  const mutation = useMutation({
    mutationFn: (data: { description: string; tags?: string[]; stage?: CriterionStage; is_public: boolean }) =>
  ...
  return (
    <Stack gap="lg" maw={600}>
      ...
      <form onSubmit={handleSubmit}>
        <Stack gap="md">
          <Switch
            label="Публичный критерий (доступен всем)"
            checked={isPublic}
