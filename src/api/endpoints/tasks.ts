...
export const getWorkspaceTasks = async (workspaceId: number): Promise<TaskResponseDTO[]> => {
	const response = await api.get<TaskResponseDTO[]>(`/api/v1/workspaces/${workspaceId}/tasks`);
...
export const createTask = async (data: TaskCreateDTO): Promise<TaskResponseDTO> => {
	const response = await api.post<TaskResponseDTO>('/api/v1/tasks', data);
	return response.data;
};

export const updateTask = async (taskId: number, data: TaskUpdateDTO): Promise<TaskResponseDTO> => {
...
export const getTaskCriteria = async (taskId: number): Promise<TaskCriteriaResponseDTO[]> => {
	const response = await api.get<TaskCriteriaResponseDTO[]>(`/api/v1/tasks/${taskId}/criteria`);
	return response.data;
};
...
export const getTaskSolutions = async (taskId: number): Promise<SolutionShortResponseDTO[]> => {
	const response = await api.get<SolutionShortResponseDTO[]>(`/api/v1/tasks/${taskId}/solutions`);
	return response.data;
};
